 ...
 * @returns {Promise<boolean>} true on success, false on failure
 */
const sendBookingConfirmation = async (guestName, guestEmail, bookingReference, checkIn, checkOut, roomType, roomCount, totalAmount) => {
  try {
    ...
    await resend.emails.send({
      ...
      html: htmlContent,
      reply_to: process.env.HOTEL_EMAIL
    });
