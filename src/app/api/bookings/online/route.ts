...
export async function POST(request: NextRequest) {
  try {
    ...
    const { createOnlineBooking } = await import('@/lib/services/bookingService')
    const { sendBookingConfirmation } = await import('@/lib/emailService')

    const body = await request.json()
    console.log('[POST /api/bookings/online] Received body:', JSON.stringify(body, null, 2))
    const { fullName, email, phone, members, roomIds, checkIn, checkOut, totalGuests, bookingSource, extraExpense, captchaToken } = body
